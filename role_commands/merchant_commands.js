const {
  ...
  async function setupMerchantBotEvents(client) {
    ...
    client.on("interactionCreate", async (interaction) => {
      ...
      if (interaction.isStringSelectMenu() && interaction.customId === "MembersSelectMenu") {
        ...
        try {
          ...
          button.setDisabled(selectedMembers[selectedMemberId].user.id === userId);
      ...
      if (interaction.isModalSubmit() && interaction.customId === 'xpModal') {
        ...
        const optionalMessage = interaction.fields.getTextInputValue('optionalMessage') || 'No message provided';
        ...
        try {
          ...
          notifyTarget(`You have been granted ${xpAmount} XP by ${interaction.user.username}. Message: ${optionalMessage}`)
        } catch (err) {
          showErrorMsg(err);
        }
      }
  ...
  async function updateSelectMenu(client) {
    try {
      await messageMerchantCommands(client);
    } catch (err) {
      showErrorMsg(err);
    }
  }
  ...
  async function messageMerchantCommands(client) {
    try {
      ...
      const roleMemberSelectMenu = new ActionRowBuilder().addComponents(await buildSelectMenu(client, ["peasant", "merchant", "knight", "noble", "lord", "king", "emperor"], "MembersSelectMenu"));
